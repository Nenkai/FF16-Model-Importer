...
using static FinalFantasyConvertTool.FontGenerator;

namespace FinalFantasyConvertTool.FNT
{
    public class SDFGenerator
    {
        ...
        public SDFGenerator(float fontSize = 36)
        {
            ...
            this.z = new double[size + 1];
            this.v = new ushort[size];
            this.gridOuter = new double[size * size];
            this.gridInner = new double[size * size];
        }
        ...
        public GlyphEntry Draw(SKFont font, char c)
        {
            ...
            byte[] rgba = new byte[(int)glyphWidth * (int)glyphHeight * 4];
        ...
        private void EDT1D(double[] grid, int offset, int stride, int length, double[] f, ushort[] v, double[] z)
        {
            ...
            for (int q = 1; q < length; q++)
            {
                ...
                {
                    int r = v[k];
                    s = (double)(f[q] - f[r] + (double)q2 - (double)r * (double)r) / (double)(q - r) / 2;
                } while (s <= z[k] && --k > -1);
            ...
            for (int q = 0; q < length; q++)
            {
                ...
                int r = v[k];
                int qr = q - r;
                grid[offset + q * stride] = f[r] + (double)qr * (double)qr;
            }
        }
    }
}
