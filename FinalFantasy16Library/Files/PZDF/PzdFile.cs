...
public class PzdFile
{
        ...
        using (var reader = new FileReader(stream))
        {
            ...
            for (int i = 0; i < PzdHeader.TextContentCount; i++)
            {
                ...
                TextContents.Add(new TextContent()
                {
                    ...
                    Sound = GetString(reader, startpos),
    ...
    public void Save(Stream stream)
    {
        ...
        using (var writer = new FileWriter(stream))
        {
            ...
            for (int i = 0; i < TextContents.Count; i++)
            {
                ...
                SaveString(TextContents[i].Sound, relative_pos);
            ...
            writer.Write(Serialization.Count); //4
            writer.Write(new uint[2 + 4]);
            ...
            foreach (var str in savedStrings)
            {
                ...
                foreach (var ofs in str.Value)
                {
                    ...
                    using (writer.TemporarySeek(abs_ofs, SeekOrigin.Begin))
                    {
    ...
    public void FromXML(string xml)
    {
        ...
        using (var stringReader = new StringReader(xml))
        {
            ...
            foreach (var c in ob.TextContents)
            {
                c.Message = c.Message.Replace(@"\x0D\x0A", "&#xA;");
                c.Message = c.Message.Replace("&#xA;", "\n");
            }
            TextContents = ob.TextContents;
            PzdHeader = ob.PzdHeader;
            Serialization = ob.Serialization;
        }
    }
}
