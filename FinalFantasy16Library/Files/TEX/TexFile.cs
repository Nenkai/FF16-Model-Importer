...
using FinalFantasy16Library.Imaging;
using FinalFantasy16Library.Imaging.Files;
using FinalFantasy16Library.Imaging.Formats;
using FinalFantasy16Library.Imaging.ImageSharp;
    ...
    private void Read(Stream stream)
    {
        using (var reader = new BinaryStream(stream))
        {
            ...
            for (int i = 0; i < TexHeader.ChunkCount; i++)
            {
                ...
                using (reader.TemporarySeek(chunks[i].DataOffset, SeekOrigin.Begin))
                {
    ...
    public void Save(Stream stream)
    {
        ...
        using (var writer = new BinaryStream(stream))
        {
            ...
            for (int i = 0; i < Textures.Count; i++)
            {
                ...
                writer.WriteUInt32(Textures[i].Flags);
                writer.WriteUInt32((uint)Textures[i].Format);
                writer.WriteUInt16(Textures[i].MipCount);
                writer.WriteUInt16(Textures[i].Width);
                writer.WriteUInt16(Textures[i].Height);
                writer.WriteUInt16(Textures[i].Depth);
                writer.WriteUInt32(0); //offset set later
                writer.WriteUInt32(Textures[i].ChunkSize);
                writer.WriteUInt32(Textures[i].Color);
                writer.WriteUInt16((ushort)chunkIndex);
                writer.WriteUInt16((ushort)Textures[i].Chunks.Count);
            ...
            {
                writer.WriteUInt32(0); //offset set later
                writer.WriteUInt32(chunk.Flags);
                writer.WriteUInt32(chunk.DecompressedSize);
                writer.WriteUInt32(chunk.MiscFlags);
            }
